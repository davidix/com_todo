...
/**
 * List Model for tasks.
 *
 * @package     Todo
 * @subpackage  Models
 */
class TodoModelTasks extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'name', 'description', 'date_done', 'status'
			);
		}
	...
	protected function populateState($ordering = 'name', $direction = 'ASC')
	{
        ...
        $this->setState('filter.published', $published);		// Set filter state for category
		$category = $this->getUserStateFromRequest($this->context.'.filter.category', 'filter_category', '');
		$this->setState('filter.category', $category);
		

		// Load the parameters.
		$params = JComponentHelper::getParams('com_todo');
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__todo_task AS a');		//Join over categories
		$query->select('todo_category.name AS ' . $db->quote('category_name'))
			->join('LEFT', '#__todo_category AS todo_category ON todo_category.id = a.category');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.name LIKE' . $s . ' OR a.description LIKE' . $s . ' OR a.date_done LIKE' . $s . ' OR a.status LIKE' . $s );
			}
		}
		...
		if ($category != "")
		{
			$query->where('a.category = ' . $db->quote($db->escape($category)));
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__todo_task AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
