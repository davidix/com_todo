 ...
 * @subpackage  Models
 */
class TodoModelCategories extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'name', 'description', 'icon', 'color'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__todo_category AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.name LIKE' . $s . ' OR a.description LIKE' . $s . ' OR a.icon LIKE' . $s . ' OR a.color LIKE' . $s );
			}
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__todo_category AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
